...
void urutkanKoleksi(Koleksi dataKoleksi[], int jumlahKoleksi) {
    for (int i = 0; i < jumlahKoleksi - 1; i++) {
        for (int j = 0; j < jumlahKoleksi - i - 1; j++) {
            Koleksi *ptr1 = &dataKoleksi[j];
            Koleksi *ptr2 = &dataKoleksi[j + 1];
...
void tampilUrut(Koleksi dataKoleksi[], int jumlahKoleksi) {
    Koleksi salinanKoleksi[jumlahKoleksi];
    // Salin data
    for (int i = 0; i < jumlahKoleksi; i++) {
        salinanKoleksi[i] = dataKoleksi[i];
    }   // Tutup kurung kurawal yang hilang
    
    urutkanKoleksi(salinanKoleksi, jumlahKoleksi);
    bersihkanLayar();
    if (jumlahKoleksi == 0) {
        cout << "Koleksi masih kosong!" << endl;
    } else {
        cout << "=== Daftar Koleksi Merchandise Anime (Terurut) ===" << endl;
        cout << "----------------------------------------------------------------------------------------------------------" << endl;
        cout << left;
        cout << setw(4) << "No" << "| ";
        ...
        for (int i = 0; i < jumlahKoleksi; i++) {
            tampilkanData(salinanKoleksi[i], i + 1);
        }
        cout << "----------------------------------------------------------------------------------------------------------" << endl;
    }
}
...
void tambahKoleksi(Koleksi dataKoleksi[], int& jumlahKoleksi, const int MAX_KOLEKSI) {
    ...
    if (jumlahKoleksi >= MAX_KOLEKSI) {
        ...
        Koleksi *pointerKeKoleksi = &dataKoleksi[jumlahKoleksi]; // Pointer ke elemen baru
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
        cout << "Nama Karakter: ";
...
void tampilkanInfo() {
    ...
    cout << "Peningkatan efiseiensi memori program" << endl;
}
    ...
    while (!kembaliKeMenuUtama) {
        ...
        switch (pilih_ubah) {
            ...
            case 1:
                ...
                break;
            case 2:
                tampilUrut(dataKoleksi, jumlahKoleksi);
                break;
            case 3:
